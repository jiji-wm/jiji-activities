...
- `tests/picker_shim.rs` — `run_picker_empty_activities_warns_and_exits_zero` integration test added: sentinel-file shim strategy (shim writes `: > "$SHIM_INVOKED"` on entry; test asserts sentinel absent) closes the shim-invocation observability gap on the zero-case path; `assert_cmd::Command::stderr(predicate)` asserts the stderr message at the process boundary.
