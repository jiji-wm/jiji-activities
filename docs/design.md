...
- [ ] Mint `CliError::PickerUnavailable(io::Error)` sharing exit code 69 with `SocketUnavailable`. Display: `"picker unavailable: {io}"`. `Error::source` returns the inner `io::Error`. Add `picker_unavailable_is_69` unit test paralleling `socket_unavailable_is_69`. (Closes Appendix C I1.)
- [ ] Swap the 5 picker call sites in `src/picker.rs` from `CliError::SocketUnavailable` to `CliError::PickerUnavailable` (`ensure_available`, `pick_one`'s spawn / stdin-pipe-missing / stdin-write-failure / wait-failure paths). Update `PICKER_MISSING_MESSAGE` to drop the redundant "command not found on $PATH" framing now that the variant prefix carries the cause; new text: `"fuzzel: not on $PATH (required for single-select picker)"`. Update the `fuzzel_missing_from_path_exits_69` integration test stderr substring.
- [ ] `is_executable_file` signature change: `&Path -> Result<bool, io::Error>`. `Ok(true)` for an executable regular file; `Ok(false)` only when `metadata` returned `NotFound` (canonical "keep walking" case); `Err(io)` for any other `io::Error` kind (`PermissionDenied`, `EIO`, `FilesystemLoop`, etc.). Add unit test pinning that `PermissionDenied` surfaces, not collapses. (Closes Appendix C C2.)
- [ ] `which_in_path` updates: `Ok(true)` from `is_executable_file` returns success, `Ok(false)` continues the loop, `Err(io)` short-circuits and returns the wrapped error to the caller. Add unit test using a tempdir with a chmod-`000` PATH entry (skip-on-root) confirming the inner error reaches the caller.
- [ ] `ensure_available` updates: map `which_in_path`'s non-`NotFound` errors to `PickerUnavailable(io)` preserving the original `io::Error` (not the synthetic `PICKER_MISSING_MESSAGE` constant). Update docstring to reflect the two-arm mapping (NotFound → synthetic message, anything else → preserved error).
- [ ] Empty-activities zero-case in `src/switch.rs::run_picker`: print `"niri-activities: no activities configured; nothing to switch to"` to stderr, return `Ok(())`. Rename test `run_picker_empty_activities_short_circuits_silently` → `run_picker_empty_activities_warns_and_exits_zero`; assert the stderr message via `assert_cmd::Command::stderr(predicate)`. (Closes Appendix C I4.)

DD updates landed alongside the code commit: §4.1 table for code 69 lists both `SocketUnavailable` and `PickerUnavailable` as carriers (one row each, shared exit code); §4.4 "Runtime dep contract" sentence acknowledges `PickerUnavailable` as the typed carrier for picker dep failures; Appendix C entries I1, C2, I4 closed out with landing references in the scribe pass.
