...
`anyhow::Error` carries chained context for stderr; the `main()` dispatcher does `match err.downcast_ref::<CliError>()` against a typed enum (`SocketUnavailable`, `PickerUnavailable`, `ActivityNotFound`, `MalformedResponse`, `CantCreate`, ...) to pick the exit code. Fallback for un-typed errors is exit 1.
